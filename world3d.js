function initWorld3D(playerData) {
  ...
  // Grass patch for the event area
  visBox(38, 0.06, 14, MT.stp, -125, 0.03, -34, true);
  ...
  function animate() {
    ...
    if (window.MYTH_CLUB_CHOICE && window.MYTH_CLUB_CHOICE !== 'none' &&
      ...
      if (clubMeetingTimer >= CLUB_MEETING_INTERVAL) {
        clubMeetingTimer = 0;
        var _boothX = window.MYTH_CLUB_CHOICE === 'robotics' ? -115 : -125;
        var _atBooth = Math.sqrt(
          Math.pow(px - _boothX, 2) + Math.pow(pz - (-34), 2)) < 6;
        if (_atBooth) {
